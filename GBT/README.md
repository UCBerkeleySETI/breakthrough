...
You can partially correct for the shape of the instrumental bandpass by doing an "on minus off over off" (subtracting an off file from an on file, and dividing by the off file, e.g. (A1 - B) / B)). Alternatively you can try fitting the bandpass shape. There are various ways of doing this - one method by intern Umran Haji is [here](filterbank_tutorial/Fitting%20Coarse%20Filterbank%20Files%20via%20Data%20Transformation%20%2B%20Splines%20and%20Fourier%20Series.ipynb).

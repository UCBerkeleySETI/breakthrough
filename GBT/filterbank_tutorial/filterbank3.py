...
Branched from https://github.com/UCBerkeleySETI/breakthrough/tree/master/GBT/filterbank_tutorial
...
def rebin(d, n_x, n_y=None):
    ...
    if d.ndim == 2:
        ...
        d = d.reshape((int(d.shape[0] / n_x), n_x, int(d.shape[1] / n_y), n_y))
        d = d.mean(axis=3)
        d = d.mean(axis=1)
    elif d.ndim == 1:
        d = d[:int(d.shape[0] / n_x) * n_x]
        d = d.reshape((int(d.shape[0] / n_x), n_x))
        d = d.mean(axis=1)
    else:
...
def generate_sigproc_header(f):
    ...
    for keyword in list(f.header.keys()):  
            if keyword == 'src_raj':
...
class Filterbank(object):
    ...
    def __init__(self, filename, f_start=None, f_stop=None, 
        ...
        n_bytes  = int(self.header['nbits'] / 8)

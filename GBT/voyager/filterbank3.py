...
Python 3 compatible class and command line utility for reading and plotting filterbank files.
...
TODO: Currently, the full filterbank file is loaded into memory. Should add a MAX_SIZE
      parameter such that really huge files aren't loaded into memory in one gulp.
      
Branched from https://github.com/UCBerkeleySETI/breakthrough/tree/master/GBT/voyager on May-26-2017
...
def grab_header(filename):
    ...
    while not eoh_found:
        ...
        if b'HEADER_START' in header_sub:
            idx_start = header_sub.index(b'HEADER_START') + len('HEADER_START')
...
def len_header(filename):
    ...
    while not eoh_found:
        ...
        header_sub_count += 1
        if b'HEADER_END' in header_sub:
            idx_end = header_sub.index(b'HEADER_END') + len('HEADER_END')
            eoh_found = True
            break
...
def parse_header(filename):
    ...
    #print header
    for keyword in list(header_keyword_types.keys()):
        bKeyword = keyword.encode('ascii')
        if bKeyword in header:
            dtype = header_keyword_types.get(keyword, 'str')
            idx = header.index(bKeyword) + len(keyword)
...
class Filterbank(object):
    ...
    def __init__(self, filename):
        ...
        # Interpret binary data, convert into numpy array
        n_bytes  = self.header['nbits'] / 8
        n_chans = self.header['nchans']
        n_ifs   = self.header['nifs']
        n_ints  = len(d) / n_chans / n_bytes / n_ifs
        ...
        try:    
            if n_ifs == 1:
                dd = dd.reshape(np.int(n_ints), n_chans)
            else:
                dd = dd.reshape(np.int(n_ints), n_ifs, n_chans)
            self.data = dd
        except ValueError:
            self.info()
        ...
        self.timestamps = np.arange(0, dd.shape[0]) * t_delt + t0
        
        # Reverse data if offset is -ve
        if f_delt < 0:
    ...
    def grab_data(self, f_start=None, f_stop=None, if_id=0):
        ...
        if self.header['nifs'] == 1:
            plot_data = self.data[:, i_start:i_stop]
        else:
            plot_data = self.data[if_id, :, i_start:i_stop]
        
        #print plot_f.shape, plot_data.shape
